...
Player::Player(float x, float y) : Entity(x, y, 14, 16, EntityType::PLAYER) {
    ...
    Sprite marioSprite;
    if (!marioSprite.load("assets/mario3.png")) {
        // Si falla, usar dimensiones por defecto
        width = 14; height = 16;
    } else {
        width = marioSprite.w;
        height = marioSprite.h;
    }
 
    // Crear animaciones (por ahora, un solo frame)
    animIdle.addFrame(marioSprite, 0.1f);
    animRun.addFrame(marioSprite, 0.1f);
    animJump.addFrame(marioSprite, 0.1f);
    
    currentAnim = &animIdle;
    currentSprite = currentAnim->getCurrentSprite();
}
