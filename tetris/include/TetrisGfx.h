...
namespace TGfx {
    ...
    void update_score(uint32_t new_score);
    void update_best(uint32_t new_best);
    void update_level(uint8_t new_level);
    void update_lines(uint16_t new_lines);
    void update_combo(uint32_t new_combo);
    void update_next_piece(uint8_t type);
    void update_hold_piece(uint8_t type, bool can_hold);

	void update_best_upper(uint32_t new_best);
	void update_score_upper(uint32_t new_score) ;
}
