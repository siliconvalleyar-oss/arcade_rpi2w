...
static uint32_t last_score = 0xFFFFFFFF;
static uint32_t last_best  = 0xFFFFFFFF;
static uint8_t  last_level = 0xFF;
static uint16_t last_lines = 0xFFFF;
static uint32_t last_combo = 0xFFFFFFFF;
...
void TGfx::draw_stats(uint32_t score, uint32_t best,
    ...
    // Combo (si > 1)
    if(combo >= 2){
        snprintf(buf,sizeof(buf),"x%lu COMBO",(unsigned long)combo);
        fill_rect(FIELD_X, FIELD_Y+2, FIELD_W, 12, COLOR_EMPTY);
        draw_string(FIELD_X+2, FIELD_Y+2, buf, NEON_GREEN, COLOR_EMPTY, 1);
    }
}
...
void TGfx::update_score(uint32_t new_score) {
    if(new_score == last_score) return;
    last_score = new_score;
    char buf[12];
    snprintf(buf, sizeof(buf), "%7lu", (unsigned long)new_score);
    int16_t x = PANEL_X+2;
    int16_t y = 142;
    fill_rect(x, y, PANEL_W-4, 10, COLOR_PANEL);
    draw_string(x, y, buf, WHITE, COLOR_PANEL, 1);
}

void TGfx::update_best(uint32_t new_best) {
    if(new_best == last_best) return;
    last_best = new_best;
    char buf[12];
    snprintf(buf, sizeof(buf), "%7lu", (unsigned long)new_best);
    int16_t x = PANEL_X+2;
    int16_t y = 172;
    fill_rect(x, y, PANEL_W-4, 10, COLOR_PANEL);
    draw_string(x, y, buf, GOLD, COLOR_PANEL, 1);
}

void TGfx::update_level(uint8_t new_level) {
    if(new_level == last_level) return;
    last_level = new_level;
    char buf[12];
    snprintf(buf, sizeof(buf), "   %3u", new_level);
    int16_t x = PANEL_X+2;
    int16_t y = 202;
    fill_rect(x, y, PANEL_W-4, 10, COLOR_PANEL);
    draw_string(x, y, buf, WHITE, COLOR_PANEL, 1);
    
    // También actualizar la barra superior (LEVEL)
    snprintf(buf, sizeof(buf), "L%02u", new_level);
    fill_rect(166, 4, 30, 10, COLOR565(5,5,20));
    draw_string(166, 4, buf, NEON_GREEN, COLOR565(5,5,20), 1);
}

void TGfx::update_lines(uint16_t new_lines) {
    if(new_lines == last_lines) return;
    last_lines = new_lines;
    char buf[12];
    snprintf(buf, sizeof(buf), "%6u", new_lines);
    int16_t x = PANEL_X+2;
    int16_t y = 222;
    fill_rect(x, y, PANEL_W-4, 10, COLOR_PANEL);
    draw_string(x, y, buf, WHITE, COLOR_PANEL, 1);
    ...
    // Barra superior
    snprintf(buf, sizeof(buf), "%04u", new_lines);
    fill_rect(196, 4, 40, 10, COLOR565(5,5,20));
    draw_string(196, 4, buf, WHITE, COLOR565(5,5,20), 1);
}

void TGfx::update_score_upper(uint32_t new_score) {
    char buf[10];
    snprintf(buf, sizeof(buf), "%06lu", (unsigned long)new_score);
    fill_rect(16, 4, 48, 10, COLOR565(5,5,20));
    draw_string(16, 4, buf, WHITE, COLOR565(5,5,20), 1);
}

void TGfx::update_best_upper(uint32_t new_best) {
    char buf[10];
    snprintf(buf, sizeof(buf), "%06lu", (unsigned long)new_best);
    fill_rect(96, 4, 48, 10, COLOR565(5,5,20));
    draw_string(96, 4, buf, GOLD, COLOR565(5,5,20), 1);
}

void TGfx::update_next_piece(uint8_t type) {
    int16_t px = PANEL_X+6, py = 32;
    fill_rect(px-2, py-2, PANEL_W-8, 40, COLOR_PANEL);
    draw_mini_piece(px, py, type, 0, false);
}

void TGfx::update_hold_piece(uint8_t type, bool can_hold) {
    int16_t px = PANEL_X+6, py = 92;
    fill_rect(px-2, py-2, PANEL_W-8, 40, COLOR_PANEL);
    if(type < 7)
        draw_mini_piece(px, py, type, 0, !can_hold);
    else
        draw_string(px, py+8, "----", COLOR565(80,80,80), COLOR_PANEL, 1);
}

void TGfx::update_combo(uint32_t new_combo) {
    if(new_combo == last_combo) return;
    last_combo = new_combo;
    fill_rect(FIELD_X, FIELD_Y+2, FIELD_W, 12, COLOR_EMPTY);
    if(new_combo >= 2) {
        char buf[20];
        snprintf(buf, sizeof(buf), "x%lu COMBO", (unsigned long)new_combo);
        draw_string(FIELD_X+2, FIELD_Y+2, buf, NEON_GREEN, COLOR_EMPTY, 1);
    }
}
