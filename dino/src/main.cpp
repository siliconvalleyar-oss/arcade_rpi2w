...
static const uint32_t in_pins[] = { BTN_FIRE_PIN, BTN_LEFT_PIN, BTN_RIGHT_PIN };
#define N_IN_PINS 3

static int gpio_init(void) {
    gpio_fd = open(GPIO_CHIP, O_RDONLY);
    if (gpio_fd < 0) { perror("open gpiochip0"); return -1; }

    struct gpiohandle_request req;
    memset(&req, 0, sizeof(req));
    req.flags = GPIOHANDLE_REQUEST_OUTPUT;
    req.lines = N_OUT_PINS;
    ...
    if (ioctl(gpio_fd, GPIO_GET_LINEHANDLE_IOCTL, &req) < 0) {
        perror("GPIO_GET_LINEHANDLE_IOCTL out"); return -2;
    }
    gpio_out_fd = req.fd;
    ...
    req.lines = N_IN_PINS;
    for (int i = 0; i < N_IN_PINS; i++) req.lineoffsets[i] = in_pins[i];
    strncpy(req.consumer_label, "dino_in", 15);
    if (ioctl(gpio_fd, GPIO_GET_LINEHANDLE_IOCTL, &req) < 0) {
        perror("GPIO_GET_LINEHANDLE_IOCTL in"); return -3;
    }
    gpio_in_fd = req.fd;
    return 0;
}
...
static void handler(int s) { (void)s; cleanup(); _exit(0); }

int hw_init(void) {
    signal(SIGINT, handler);
    signal(SIGTERM, handler);
    if (gpio_init() < 0) return -1;
    if (spi_init() < 0) { cleanup(); return -2; }
    RST_HIGH(); DC_LOW(); BL_LOW();
    return 0;
}

int main(void) {
    fprintf(stderr, "=== DINO RUNNER ===\n");
    if (hw_init() < 0) { fprintf(stderr, "hw_init failed\n"); return 1; }
    init_display();
    delay_ms(50);
    BL_HIGH();
